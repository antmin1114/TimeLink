...
- 예약 가능 시간 생성 방식을 단일 슬롯 생성에서 '예약 가능 시간 범위 등록' 방식으로 변경
- 선택한 시간 범위를 30분 또는 60분 단위의 TimeSlot으로 자동 분할 생성하도록 정책 수정
- Guest 예약 시 하나의 슬롯이 아닌 여러 개의 연속된 슬롯(slotIds)을 선택하는 방식으로 정책 변경
- 슬롯 선택 UX를 시작/종료 슬롯 선택 기반의 자동 범위 선택 방식으로 변경
...
Host는 단일 슬롯을 생성하는 것이 아니라 예약 가능한 시간 범위를 등록한다.

앱은 선택한 시간 범위를 30분 또는 60분 단위의 TimeSlot으로 자동 분할하여 생성한다.
...
- 슬롯 단위는 30분 또는 60분이다.
- 슬롯 단위는 예약 시간 길이가 아니라 슬롯 분할 기준이다.
- 이미 존재하는 슬롯과 겹치는 경우 해당 슬롯은 생성하지 않는다.
- 생성된 슬롯은 각각 독립적인 TimeSlot이다.
- Host는 생성된 개별 슬롯을 비활성화할 수 있다.
